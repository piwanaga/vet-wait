...
const Queue = ({ items, clearItems }) => {
    ...
    const renderTextBtn = i => {
        return textSent.includes(i.id) ? 
            <button
                disabled
                className= 'disabled text-blue-400 text-xs tracking-wider bg-blue-200 bg-opacity-20 px-2 py-1 ml-2 sm:ml-4 md:ml-8 rounded-lg focus:outline-none focus:ring focus:ring-offset-2 focus:ring-gray-200 focus:ring-opacity-50 transform transition' 
                > 
                text sent
                <span> 
            ...
            <button
                className= 'text-white bg-blue-400 text-xs tracking-wider px-2 py-1 ml-2 sm:ml-4 md:ml-8 rounded-lg border hover:bg-blue-300  focus:outline-none focus:ring focus:ring-offset-2 focus:ring-blue-400 focus:ring-opacity-50 active:bg-blue-500 transform transition'
            ...
            <table className='table-fixed w-full border-collapse'>
                <thead className='bg-gray-100'>
                    <tr className='border-b'>
                        <th className='hidden sm:table-cell w-10 md:w-20 lg:w-16 text-left text-sm sm:text-lg font-semibold px-4 py-4 tracking-wide text-gray-900'>Pos</th>
                        <th className='hidden sm:table-cell w-20 text-left text-sm sm:text-lg font-semibold px-4 py-4 tracking-wide text-gray-900'>Owner</th>
                        <th className='w-16 sm:w-14 lg:w-20 text-left text-sm sm:text-lg font-semibold px-4 py-4 tracking-wide text-gray-900'>Pet</th>
                        <th className='w-36 sm:w-36 md:w-40 lg:w-28 text-left text-sm sm:text-lg font-semibold px-4 py-4 tracking-wide text-gray-900'>Phone</th>
                        <th className='w-20 sm:w-24 md:w-28 text-left text-sm sm:text-lg font-semibold px-4 py-4 tracking-wide text-gray-900'>
                            Checked In
                        </th>
                    </tr>
                </thead>
                ...
                {items.map((i, idx) => (
                    <tr key={i.id} className='hover:bg-gray-50'>
                        ...
                        <td className='hidden sm:table-cell border px-3 py-3 text-gray-900 text-xs sm:text-sm overflow-auto'>{i.firstName} {i.lastName}</td>
